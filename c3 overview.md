---
title: Overview
summary: An overview of the C3 programming language and its features.
weight: 2
---

## Introduction

This article is an introduction to the C3 Programming Language. You're assumed to have basic knowledge of programming concepts such as variables, statements, and types. You will see how familiar things are done in C3, while also being introduced to new concepts.

If you haven't already installed C3, then you can read how in the  
https://c3-lang.org/getting-started/prebuilt-binaries/

For example for Windows -  
Download the compiler binary for C3 from github release page:  
https://github.com/c3lang/c3c/releases  
Download "c3-windows.zip" 
Extract the zip file to some location like:  
D:\compilers\c3  
In the c3 folder the "c3c.exe" is the compiler binary.
For easier use set the path to this binary as a global environment variable.  
On Windows 11 right click on Desktop -> Personalize -> System -> About -> Advanced system settings -> Environment Variables -> System Variables -> Path (Select entry) -> Edit -> New -> Add this - D:\compilers\c3 (this path to folder where you extracted the c3-windows.zip) -> ok -> ok

## Hello, World!

To begin this tour, let us start with the famous C language "hello world" program:
...
```
Save this code to a `.c3` file, then compile it using -   
c3c compile hello_world.c3  
Then run the exe by double clicking it.

You can compile and run in one command by using the following command:
```txt
c3c compile-run hello_world.c3
```
If you put this command in a batch file (.bat) then put pause also in next line after command to stop console window from exting immediately after running the program to see the output.  
```txt
c3c compile-run hello_world.c3  
pause
```
...
The length of a string can be found using the built-in [`len`](https://pkg.C3-lang.org/base/builtin/#len) proc:
```c3
len("Foo")
len(some_string)
```

If the string passed to [`len`](https://pkg.C3-lang.org/base/builtin/#len) is a compile-time constant, the value from [`len`](https://pkg.C3-lang.org/base/builtin/#len) will be a compile-time constant.
...
In C3, if a number constant can be represented by a type without precision loss, it will automatically convert to that type.
```c3
...
For more information regarding value declarations in general, please see the [C3 FAQ](/docs/faq) and Ginger Bill's article [On the Aesthetics of the Syntax of Declarations](https://www.gingerbill.org/article/2018/03/12/on-the-aesthetics-of-the-syntax-of-declarations/).


## Packages

C3 programs consist of packages. A package is a directory of C3 code files, all of which have the same package declaration at the top. Execution starts in the package's `main` procedure.
...
A package is a directory of C3 code files, all of which have the same package declaration at the top, e.g. `package main`. Each .C3 file _must_ have the same package name. A directory cannot contain more than 1 package.
...
### `for` statement
C3 has only one loop statement, the `for` loop.
...
C3's `if` statements do not need to be surrounded by parentheses `( )` but braces `{ }` or `do` are required.
...
### `switch` statement
A switch statement is another way to write a sequence of if-else statements. In C3, the default case is denoted as a case without any expression.

```c3
switch arch := C3_ARCH; arch {
...
```
C3's `switch` is like the one in C or C++, except that C3 only runs the selected case. This means that a `break` statement is not needed at the end of each case. Another important difference is that the case values need not be integers nor constants.
...
**Note:** The `defer` construct in C3 differs from Go's `defer`, which is function-exit and relies on a closure stack system.
...
The `when` statement is very useful for writing platform specific code. This is akin to the `#if` construct in C's preprocessor. However, in C3, it is type checked.
...
## Procedures
In C3, a procedure is something that can do work, which some languages call _functions_ or _methods_. A procedure literal in C3 is defined with the `proc` keyword:
...
For more information regarding value declarations in general, please see the [C3 FAQ](/docs/faq).
...
By default, C3 procedures use the `"C3"` calling convention. This calling convention is the same as C, however it differs in a couple of ways:
...
The promotion is enabled by the fact that all parameters are immutable in C3, and its rules are consistent for a given type and platform and can be relied on since they are part of the calling convention.
...
### Multiple results
A procedure in C3 can return any number of results. For example:
```c3
...
Unlike other languages, C3 provides the ability to explicitly overload procedures:
```c3
...
The design goals of C3 were explicitness and simplicity. Implicit procedure overloading complicates the scoping system. In C++, you cannot nest procedures within procedures, so all procedure look-ups are done at the global scope. In C3, procedures can be nested within procedures and, as a result, determining which procedure should be used, in the case of implicit overloading, is complex.
...
**Note:** The C3 `string` type stores the pointer to the data and the length of the string. `cstring` is used to interface with foreign libraries written in/for C that use zero-terminated strings.
...
### Untyped types
In the C3 type system, certain expressions will have an "untyped" type. An untyped type can implicitly convert to a "typed" type.
```c3
...
There are a few built-in constants and values in C3 which have different uses:
...
```
This is the default behaviour in C, whilst the default behaviour in C3 is to zero the memory.
...
For the full list of builtin-procedures, see the documentation for [package builtin](https://pkg.C3-lang.org/base/builtin/).

There are two kinds of built-in procedures in C3:

* Compiler defined
* Core library defined

### `string` type
As previously mentioned the C3 `string` type is just a `rawptr` + `len`.
...
All procedures are [documented](https://pkg.C3-lang.org/core/strings/) and can be easily understood with code examples.
...
Iteration through runes is preferred since C3 strings are ***UTF8***. Most core library procedures will be addressed by `*_byte` if they do input/output an index in *byte*.
...
A `cstring` is easily convertible to an C3 `string`. However, to convert a `string` to a `cstring`, it requires allocations if the value is not constant.
...
Here is a short list of possible type conversions - including whether they *copy* or *alias*. This is important to understand since ***C3*** gives you the possibility to keep *allocations* to a low degree.
...
The built-in [`len`](https://pkg.C3-lang.org/base/builtin/#len) proc returns the array's length.
```c3
x: [5]int
#assert(len(x) == 5)
```
...
#### Array programming
C3's fixed length arrays support [array programming](https://en.wikipedia.org/wiki/Array_programming).
...
**n.b.** C3 also supports [`matrix` types](/docs/overview/#matrix-type).
...
The built-in [`len`](https://pkg.C3-lang.org/base/builtin/#len) proc returns the slice's length.
```c3
x: []int = ...
length_of_x := len(x)
```
...
Along with the built-in proc [`len`](https://pkg.C3-lang.org/base/builtin/#len), dynamic arrays also have [`cap`](https://pkg.C3-lang.org/base/builtin/#cap) which can used to determine the dynamic array's current underlying capacity.

#### Appending to a dynamic array

It is common to append new elements to a dynamic array; this can be done using the built-in [`append`](http://pkg.C3-lang.org/base/builtin/#append) proc.
```c3
x: [dynamic]int
append(&x, 123)
...
* [`pop`](https://pkg.C3-lang.org/base/builtin/#pop) pops the last element of the array
* [`unordered_remove`](https://pkg.C3-lang.org/base/builtin/#unordered_remove) removes an element at a specific index. *Unordered* means it is `O(1)`, since it swaps the last element to the removed location - making the array be *sorted* differently.
* [`ordered_remove`](https://pkg.C3-lang.org/base/builtin/#ordered_remove) removes an element at a specific index. *Ordered* means it will move all elements after the index downwards with a `copy` - ensuring elements remain in the same order.
...
Other variants can be found in the [built-in procedures](https://pkg.C3-lang.org/base/builtin) documentation.
...
Slices and dynamic arrays can be explicitly allocated with the built-in [`make`](http://pkg.C3-lang.org/base/builtin/#make) proc.
...
Slices and dynamic arrays can be deleted with the built-in [`delete`](http://pkg.C3-lang.org/base/builtin/#delete) proc.
...
**Note:** There is no automatic memory management in C3.

#### Clearing a dynamic array

Instead of deleting the array you often want to simply [clear](https://pkg.C3-lang.org/base/builtin/#clear) the dynamic array. This will set the length `len()` to be `0`, while the capacity `cap` remains the same.
...
* [resize](https://pkg.C3-lang.org/base/builtin/#resize) will try to resize memory of a passed dynamic array to the requested element count (setting the `len`, and possibly `cap`).
* [reserve](https://pkg.C3-lang.org/base/builtin/#reserve) will try to reserve memory of a passed dynamic array to the requested element count (setting the `cap`).
* [shrink](https://pkg.C3-lang.org/base/builtin/#shrink) will shrink the capacity of a dynamic array down to the current length, or the given capacity.
...
C3 supports a fixed capacity dynamic array `[dynamic; N]T`. It implements most dynamic array procedures while being able to remain on the stack.
...
n.b. This replaces the `core` library container dynamic array [Small_Array(N, T)](https://github.com/C3-lang/C3/blob/master/core/container/small_array/small_array.C3).
...
Enums can be trivially `for` looped in C3. This way we can loop through the entire `enum` and do things like printing or inserting into an *Enumerated Array*.
...
### Pointers
C3 has pointers. A pointer is a memory address of a value. The type `^T` is a pointer to a `T` value. Its zero value is `nil`.
```c3
p: ^int
```
...
**Note:** C programmers may be used to using `*` to denote pointers. In C3, the `^` syntax is borrowed from Pascal. This is to keep the convention of the type on the left and its usage on the right:
```c3
p: ^int // ^ on the left
x := p^ // ^ on the right
```

**Note:** Unlike C, C3 has no pointer arithmetic. If you need a form of pointer arithmetic, please use the `ptr_offset` and `ptr_sub` procedures in the `"core:mem"` package.

### Structs
A `struct` is a record type in C3. It is a collection of fields. Struct fields are accessed by using a dot:
```c3
Vector2 :: struct {
    x: f32,
    y: f32,
...
Within C3's core library, the standard convention is to store a `key` that denotes the package and then a subsequence `"value"`. For example, `json` keys are processed and used by `core:encC3g/json` package, `fmt` keys are processed by `core:fmt`.
...
### Unions
A `union` in C3 is a discriminated union, also known as a tagged union or sum type. The zero value of a union is `nil`.
```c3
Value :: union {
    bool,
    i32,
...
### Maps
A `map` maps keys to values. The zero value of a map is `nil`. A `nil` map has no keys. The built-in [`make`](http://pkg.C3-lang.org/base/builtin/#make) proc returns an initialized map using the current [context](#implicit-context-system), and [`delete`](http://pkg.C3-lang.org/base/builtin/#delete) can be used to delete a map.
...
A `bit_field` is a record type in C3 that is akin to a bit-packed struct. **Note:** `bit_field` is __not__ equivalent to `bit_set` as it has different semantics and use cases. `bit_field` fields are accessed by using a dot:
```c3
...
* C3's `bit_field` and C's bit-fields might not be compatible
    * C3's `bit_field`s have a well defined layout (Least-Significant-Bit)
...
#### Calling conventions
C3 supports the following calling conventions:

* **C3** - default convention used for an C3 **proc**. It passes all parameters larger than 16 bytes by reference and passes an implicit `context` pointer on each call. (**Note:** This is subject to change)
* **contextless** - This is the same as **C3** but without the implicit `context` pointer.
...
The default calling convention is **C3**, unless it is within a `foreign` block, where it is then **cdecl**.
...
### `typeid` type
A `typeid` is a unique identifier for an C3 type. This construct is used by the `any` type to denote what the underlying data's type is.
```c3
a := typeid_of(bool)
i: int = 123
...
Multi-Pointers in C3 are a way to describe [`foreign`](#foreign-system) (C-like) pointers which act like arrays (pointers that map to multiple items). The type `[^]T` is a multi-pointer to T value(s). Its zero value is `nil`.
...
The main purpose of this type is to aid with `foreign` code and act as a way to auto-document functionality and allow for easier transition to C3 code, especially converting pointers into slices.
...
To be complete with SOA slices, C3 also supports SOA dynamic arrays.
...
SOA is not just useful for high performance scenarios but also for everyday tasks which are normally only achievable in higher level languages. [`soa_zip`](http://pkg.C3-lang.org/base/builtin/#soa_zip) is a built-in procedure which allows the user to treat multiple slices as if they are part of the same data structures, utilizing the power of SOA.
...
[`soa_unzip`](http://pkg.C3-lang.org/base/builtin/#soa_unzip) is a built-in procedure which allows the user to recover the slices from an `#soa` slice.
...
A `matrix` is a [mathematical type](https://wikipedia.org/wiki/Matrix_(mathematics)) built into C3. It is a regular array of numbers, arranged in rows and columns.
...
Casting a matrix to another matrix is allowed as long as they share the same element type and the number of elements (rows\*columns). Matrices in C3 are stored in column-major order, which means the casts will preserve this element order.
...
The default internal representation of a matrix in C3 is stored in column-major format
...
[raw_data](https://pkg.C3-lang.org/base/builtin/#raw_data) is a built-in procedure which returns the underlying data of a built-in data type as a [Multi-Pointer](#multi-pointers).
...
foo_3 :: proc() -> (ok: bool) {
    // `or_return` also supports the ok semantics common in C3 code.
...
Only job #5 completes, because it has only 1 task left and its `error_has_occurred` is the default value, which is `false` in this case, as every variable in C3 is implicitly zeroed out unless made explicit.
...
Your .C3 files can have a magic suffix that will cause the compiler to either include or exclude them based on the target platform or architecture, or both.

For example, `foobar_windows.C3` would only be compiled on Windows, `foobar_linux.C3` only on Linux, and `foobar_windows_amd64.C3` only on Windows AMD64.
...
| Name                              | Description                                                                                                                                                                                 |
|-----------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `C3_ARCH`                       | An enum value indicating what the CPU architecture of the target is. (`.amd64`, `.i386`, `.arm32`, `.arm64`, `.wasm32`, `.wasm64p32`, `.riscv64`)                                           |
| `C3_ARCH_STRING`                | A string indicating what the CPU architecture of the target is.  (`"amd64"`, `"i386"`, `"arm32"`, `"arm64"`, `"wasm32"`, `"wasm64p32"`, `"riscv64"`)                                       |
| `C3_BUILD_MODE`                 | An enum value indicating what type of compiled output the user desires. (`.Executable`, `.Dynamic`, `.Static`, `.Object`, `.Assembly`, or `.LLVM_IR`)                                       |
| `C3_BUILD_PROJECT_NAME`         | Name of the folder that contains the entry point.                                                                                                                                           |
| `C3_COMPILE_TIMESTAMP`          | An i64 containing the time at which the executable was compiled, in nanoseconds. This is compatible with the `time.Time` type, i.e. `time.Time{_nsec=C3_COMPILE_TIMESTAMP}`               |
| `C3_DEBUG`                      | `true` if the `-debug` command line switch is passed, which enables debug info generation.                                                                                                  |
| `C3_DEFAULT_TO_NIL_ALLOCATOR`   | `true` if the `-default-to-nil-allocator` command line switch is passed, which sets the initial allocator to an allocator that does nothing.                                                |
| `C3_DEFAULT_TO_PANIC_ALLOCATOR` | `true` if the `-default-to-panic-allocator` command line switch is passed, which sets the initial allocator to an allocator that panics if allocated from.                                  |
| `C3_DISABLE_ASSERT`             | `true` if the `-disable-assert` command line switch is passed, which removes all calls to `assert` from the compilation.                                                                    |
| `C3_ENDIAN`                     | An enum value indicating the endianness of the target. (`.Little`, `.Big`)                                                                                                                  |
| `C3_ENDIAN_STRING`              | A string indicating the endianness of the target. (`"little"`, `"big"`)                                                                                                                     |
| `C3_ERROR_POS_STYLE`            | An enum value set using the `-error-pos-style` switch, indicating the source location style used for compile errors and warnings. (`.Default` (C3), `.Unix`)                              |
| `C3_FOREIGN_ERROR_PROCEDURES`   | `true` if the `-foreign-error-procedures` command line switch is passed, which inhibits generation of runtime error procedures, so that they can be in a separate compilation unit.         |
| `C3_MICROARCH_STRING`           | A string describing the microarchitecture used for code generation. Can be set using the `-microarch` command line switch. E.g. `"sandybridge"`.                                            |
| `C3_MINIMUM_OS_VERSION`         | An integer value representing the minimum OS version set using `-minimum-os-version`. Calculated as `major * 10_000 + minor * 100 + revision`, and defaults to `0` if not specified.        |
| `C3_NO_BOUNDS_CHECK`            | `true` if the `-no-bounds-check` command line switch is passed, which disables bounds checking at runtime.                                                                                  |
| `C3_NO_CRT`                     | `true` if the `-no-crt` command line switch is passed, which inhibits linking with the C Runtime Library, a.k.a. LibC.                                                                      |
| `C3_NO_ENTRY_POINT`             | `true` if the `-no-entry-point` command line switch is passed, which makes the declaration of a `main` procedure optional.                                                                  |
| `C3_NO_RTTI`                    | `true` if the `-no-rtti` command line switch is passed, which inhibits generation of full Runtime Type Information.                                                                         |
| `C3_NO_TYPE_ASSERT`             | `true` if the `-no-type-assert` command line switch is passed, which disables type assertion checking program wide.                                                                         |
| `C3_OPTIMIZATION_MODE`          | An enum value indicating the optimization level selected using the `-o` command line switch. (`.None`, `.Minimal`, `.Size`, `.Speed`, `.Aggressive`)                                        |
| `C3_OS`                         | An enum value indicating what the target operating system is.                                                                                                                               |
| `C3_OS_STRING`                  | A string indicating what the target operating system is.                                                                                                                                    |
| `C3_PLATFORM_SUBTARGET`         | An enum value indicating the platform subtarget, chosen using the `-subtarget` switch. (`.Default`, `.iOS`, `.Android`)                                                                     |
| `C3_ROOT`                       | Path to the folder containing the C3 compiler executable.                                                                                                                                 |
| `C3_SANITIZER_FLAGS`            | A bit_set indicating the sanitizer flags set using the `-sanitize` command line switch. (`.Address`, `.Memory`, and `.Thread`).                                                             |
| `C3_TEST`                       | `true` if the code is being compiled via an invocation of `C3 test`.                                                                                                                      |
| `C3_USE_SEPARATE_MODULES`       | `true` by default, false if the `-use-single-module` command line switch is passed to force a unity build. By default each package is compiled into an object file, then linked together.   |
| `C3_VALGRIND_SUPPORT`           | `true` if Valgrind integration is supported on the target.                                                                                                                                  |
| `C3_VENDOR`                     | String which identifies the compiler being used. The official compiler sets this to `"C3"`.                                                                                               |
| `C3_VERSION`                    | A string that represents the C3 compiler version being used. (e.g: `dev-2023-04`)                                                                                                         |
| `C3_VERSION_HASH`               | A string containing the Git hash part of the C3 version. Empty if `.git` could not be detected at the time the compiler was built.                                                        |
| `C3_WINDOWS_SUBSYSTEM`          | An enum value indicating the desired Windows PE subsystem, chosen using the `-subsystem` switch. (`.Console` (default), `.Windows`, or `.Unknown` on non-Windows platforms.                 |
| `C3_WINDOWS_SUBSYSTEM_STRING`   | A string indicating the desired Windows PE subsystem, chosen using the `-subsystem` switch. (`"CONSOLE"` (default), `"WINDOWS"`, or `""` on non-Windows platforms.                          |
| `__C3_LLVM_F16_SUPPORTED`       | `true` if LLVM supports the `f16` type.                                                                                                                                                     |

See the [tracking allocator](#tracking-allocator) for an example of something that uses the `C3_DEBUG` constant in a `when` statement.
...
For the sake of demonstration, let's take POSIX: You could use `foobar_unix.C3`, which has no special meaning to the compiler at all, and use a tag in the file itself.
...
##### `#+test`
The file is completely ignored from parsing and type checking _EXCEPT_ during `C3 test`.
...
In each scope, there is an implicit value named `context`. This `context` variable is local to each scope and is implicitly passed by pointer to any procedure call in that scope (if the procedure has the C3 calling convention).
...
To see what the implicit `context` value contains, please see the definition of the `Context` struct in [package runtime](https://github.com/C3-lang/C3/blob/master/base/runtime/core.C3).

### Allocators
C3 is a manual memory management based language. This means that C3 programmers must manage their own memory, allocations, and tracking. To aid with memory management, C3 has huge support for custom allocators, especially through the implicit `context` system.
...
All allocations in C3 are preferably done through allocators. The core library of C3 takes advantage of allocators through the implicit `context` system. The following call:
...
The allocator from the `context` is implicitly assigned as a default parameter to the built-in procedure [`new`](http://pkg.C3-lang.org/base/builtin/#new).
...
* [`new`](http://pkg.C3-lang.org/base/builtin/#new) - allocates a value of the type given. The result value is a pointer to the type given.
...
* [`new_clone`](http://pkg.C3-lang.org/base/builtin/#new) - allocates a clone of the value passed to it. The resulting value of the type will be a pointer to the type of the value passed.
...
* [`make`](http://pkg.C3-lang.org/base/builtin/#make) - allocates memory for a backing data structure of either a [slice](#slices), [dynamic array](#dynamic-arrays), or [map](#maps).
...
* [`free`](http://pkg.C3-lang.org/base/builtin/#free) - frees the memory at the pointer given. **Note:** only free memory with the allocator it was allocated with.
...
* [`free_all`](http://pkg.C3-lang.org/base/builtin/#free_all) - frees all the memory of the context's allocator (or given allocator). **Note:** not all allocators support this procedure.
...
* [`delete`](http://pkg.C3-lang.org/base/builtin/#delete) - deletes the backing memory of a value allocated with make or a string that was allocated through an allocator.
...
To see more uses of allocators and allocation-related procedures, please see [`package mem`](https://github.com/C3-lang/C3/tree/master/core/mem) in the core library.
...
Procedures which do not use the `"C3"` calling convention must explicitly assign the `context` if something within its body requires it.
...
error_callback :: proc "c" (code: i32, desc: cstring) {
    ...
    fmt.println(desc, code) // fmt.* calls use the C3 calling convention
}
...
To see more uses of loggers, please see [`package log`](https://github.com/C3-lang/C3/tree/master/core/log) in the core library.

## Foreign system
It is sometimes necessary to interface with foreign code, such as a C library. In C3, this is achieved through the `foreign` system. You can "import" a library into the code using the same semantics as a normal import declaration:
```c3
...
The C3 compiler can also automatically build and link imported assembly files.
...
For examples of how C3 uses this feature, see `base/runtime/entry_*.asm`.
...
If a library exports global variables, you can import those into C3 as well.
...
As described in the [Foreign System](#foreign-system) we often want to use existing `C` libraries. C3 has a wide collection of maintained [bindings and ports](https://github.com/C3-lang/C3/tree/master/vendor).
...
**Note:** Within the C3 code base and documentation, the nickname "parapoly" is usually used.

### Explicit parametric polymorphism
Explicit parametric polymorphism means that the types of the parameters of a `proc` or of the data fields of a `struct` (when intended to potentially be used with multiple possible types) must be explicitly provided. This is similar to how C++ allows the use of `template`s to fill out the body of a procedure or data structure with the types that are given at compile-time as input to the `template` parameters, but in C3 explicit parametric polymorphism is safer and cleaner to work with. 
...
**Note:** Within the C3 code base and documentation, the name "polymorphic name" is usually used.
...
* [`@(builtin)`](#builtin) --- marks builtin declarations in C3's `base:runtime` package. Cannot be used in user code
...
Marks builtin procs in C3's "base:runtime" package. Cannot be used in user code.
...
Mark a procedure as deprecated. Running `C3 build/run/check` will print out the message for each usage of the deprecated proc.
```c3
...
Specifies whether this declaration is an implementation of an Objective-C class or not on the C3-side.
...
Adds custom a debug watch window rendering for the RAD Debugger. If no custom view rule as a string is set, then the C3 compiler will generate a specific view from the struct fields tags (reading the `fmt:"..."` style declarations).
...
Allows procedures with the attribute `@(test)` to be run with the command `C3 test` directly.
...
Directives are a way of extending the core behaviour of the C3 programming language. They have the form `#directive_name`.
...
`#caller_location` sets a parameter's default value to the location of the code calling the procedure. The location value has the type [`Source_Code_Location`](https://pkg.C3-lang.org/base/runtime/#Source_Code_Location). `#caller_location` may only be used as a default value for procedure parameters.
...
main :: proc() {
    ...
    // C:/some/dir/example_caller_location.C3(11:2)
...
The `#bounds_check` and `#no_bounds_check` flags control C3's built-in bounds checking of arrays and slices. Any statement, block, or function with one of these flags will have their bounds checking turned on or off, depending on the flag provided. Valid uses of these flags include:
```c3
...
By default, the C3 compiler has bounds checking enabled program-wide where applicable, and it may be turned off by passing the `-no-bounds-check` build flag.
...
By default, the C3 compiler has type assertions enabled program-wide where applicable, and they may be turned off by passing the `-no-type-assert` build flag.
...
Returns `true` or `false` if the file at the given path exists. If the path is relative, it is accessed relative to the C3 source file that references it.
...
When used within a [`defer`](#defer-statement) statement, this directive returns a [`runtime.Source_Code_Location`](https://pkg.C3-lang.org/base/runtime/#Source_Code_Location) of the point at which the control flow triggered execution of the `defer`. This may be a `return` statement or the end of a scope.
```c3
package main
...
Returns a [`runtime.Source_Code_Location`](https://pkg.C3-lang.org/base/runtime/#Source_Code_Location). Can be called with no parameters for current location, or with a parameter for the location of the variable/proc declaration.
```c3
foo :: proc() {}
...
```c3
DEBUG_LOG_SIZE :: 1024 when C3_DEBUG else 0
```
...
C3 has multiple return values, so `Maybe(T)` is used less frequently or rarely in the `core` library. Instead of doing `-> Maybe(int)` you could transform it to `-> (int, bool)`.
...
C3 is a strongly and `distinct`ly typed language by default. It has very few implicit type conversions compared to many other languages.
...
If you are unable to find the information you need on this overview page, the [FAQ](https://C3-lang.org/docs/faq/), the [wiki](https://github.com/C3-lang/C3/wiki), the [example repository](https://github.com/C3-lang/examples) (which contains demos for using the vendor libs, e.g. [Tetris in raylib](https://github.com/C3-lang/examples/blob/master/raylib/tetroid/raylib_tetroid.C3)), or the [package documentation](https://pkg.C3-lang.org/), then you will likely find it very helpful to explore your C3 source code directory, especially the files contained in the following five subdirectories: `examples\demo`, `tests`, `core`, `vendor`, and `misc\old_demos`.  

The following list describes why each of these may be useful:

- The `examples\demo\demo.C3` file contains ~2500 lines of C3 code that demonstrate many of C3's language features and provide commentary on some of them. This file contains many examples that cannot be found anywhere in the Overview nor in the other documentation.
- The `tests` folder provides unit tests for many of C3's features and libraries, whereby reading these files you can gain a more nuanced view of exactly how things are intended or expected to behave. Tests are essentially automatically verified documentation. Tests often cover details that normal documentation doesn't even mention. Tests are also guaranteed to be up-to-date (if regularly run), unlike more informal documentation.
- The `core` and `vendor` folders contain the bindings and implementation code for C3's standard library and various multimedia libraries respectively. **Even if you don't understand the implementation of something, reading the code can still greatly improve your chances of guessing what exactly it really does.** C3 is a straightforward and legible language with relatively little magic compared to other modern languages. Reading source code can enable you to work around missing documentation much more effectively. This is a very useful thing to keep in mind for working with any under-development language or library effectively.
- `misc\old_demos` may contain examples that are different from the main up-to-date `demo.C3` file contained in `examples\demo`. This may provide additional context for C3's design and features. The other resources are probably better though.
...
C3's documentation may be sparse at times, but this sparsity can be worked around effectively by using the above listed resources. *Don't forget they exist.*
...
Furthermore, even if C3 doesn't yet have a feature or library that you need, the fact that C3 has bindings to C's standard library (which are available in [the libc package](https://pkg.C3-lang.org/core/c/libc/) via `import "core:c/libc"`) means that any algorithms, example code, documentation, or books originally written for C can also be used to accomplish what you need in C3. You can then optionally write C3 wrappers around that code to bridge the idiomatic gap better or else just use the C-like code directly. You can also bind to code from any libraries or languages that `foreign` can bind to (see [the foreign section](https://C3-lang.org/docs/overview/#foreign-system) for more info).

Thus, when necessity calls for it, **the entire literature and community of C is still (in effect) available to you in C3 as well**. So, any time you are tempted to think "I can't implement this in C3 yet because there's not enough documentation" remind yourself that all (or almost all) of C is still available to you, but through a cleaner namespaced interface. C3 is also similar to C anyway, so direct translations of C to C3 (without even using the `libc` bindings) may still be relatively easy. In effect, any tutorial or book on C can also be thought of as an C3 resource too in that sense!

You can also make your experience navigating C3 code easier by installing a [syntax highlighter](https://en.wikipedia.org/wiki/Syntax_highlighting) and/or a [language server](https://en.wikipedia.org/wiki/Language_Server_Protocol), which do exist for some editors (e.g. VS Code, Vim, Sublime Text, and Emacs) and work well enough to make using C3 comfortable. See [the list of text editors that have support for C3 code](https://github.com/C3-lang/C3/wiki/C3-Libs#editor-support) for known examples.

Finally, you can ask for information from [the C3 community on Discord](https://discord.gg/vafXTdubwr) or elsewhere. There is also [an old Handmade Network forum](https://C3.handmade.network/forums) that is no longer in use and doesn't have many threads, but may still have a small amount of useful info on it.

There's a collection of other [libraries created by the C3 community](https://github.com/C3-lang/C3/wiki/C3-Libs) that may also be helpful.
